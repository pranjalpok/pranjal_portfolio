...
const VISIBLE = 2; // most recent roles always shown; the rest sit behind a toggle

function Role({ e, current }) {
  return (
    <li className="pl-7 relative">
      <span className={`absolute -left-[9px] top-1.5 w-4 h-4 rounded-full border-2 border-page ${current ? "bg-bill-tint" : "bg-mallard"}`} />
...
export default function Experience() {
  ...
  const recent = experience.slice(0, VISIBLE);
    ...
    <section id="experience" className="py-20 lg:py-24 scroll-mt-16">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <ol className="relative border-l-2 border-panel-border ml-2 space-y-9">
          {recent.map((e, i) => <Role key={e.id} e={e} current={i === 0} />)}
          {showAll && earlier.map((e) => <Role key={e.id} e={e} />)}
        </ol>
        {earlier.length > 0 && (
          <button
            onClick={() => setShowAll((v) => !v)}
            aria-expanded={showAll}
            className="mt-8 ml-2 text-sm font-medium text-mallard-deep border border-mallard/40 rounded-full px-4 py-2 hover:bg-mallard/10 transition-colors"
          >
            {showAll ? "Hide earlier roles ↑" : `Show earlier roles (${earlier.length}) ↓`}
          </button>
        )}
      </div>
    </section>
